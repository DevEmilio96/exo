...
class Topology:
  ...
  def update_edge(self, node1_id: str, node2_id: str, latency: float = 0.0, throughput: float = 0.0):
    ...
    self.throughputs[(node1_id, node2_id)] = self.throughputs[(node2_id, node1_id)] = throughput
  ...
  def __str__(self):
    ...
    throughput_str = ", ".join(f"{(node1, node2)}: {throughput}" for (node1, node2), throughput in self.throughputs.items())
    return f"Topology(Nodes: {{{nodes_str}}}, Edges: {{{edges_str}}}, Ring: {ring_str}, Latencies: {{{latency_str}}}, Throughputs: {{{throughput_str}}})"